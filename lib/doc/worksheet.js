...
Please leave feedback at https://github.com/exceljs/exceljs/discussions/2575`
  ...
  _deriveTableRows(table) {
    // Tables don't store row data in the table definition / xml - the values
    // live in the worksheet's cells.
    // Table XML stores only `ref` + column metadata; values live in the sheet.
    // Derive `rows` from sheet content (via `ref`).
    if (!table.ref) {
      return [];
    }
    let top;
    ...
    let right;
    try {
      ({top, left, bottom, right} = colCache.decode(table.ref));
    } catch (e) {
      return [];
    }
    const headerRowCount = table.headerRow ? 1 : 0;
    const totalsRowCount = table.totalsRow ? 1 : 0;
    const dataRowCount = Math.max(0, bottom - top + 1 - headerRowCount - totalsRowCount);
    const width = right - left + 1;
    const rows = [];
    for (let i = 0; i < dataRowCount; i++) {
      const sheetRow = this.getRow(top + headerRowCount + i);
      const rowValues = [];
      for (let j = 0; j < width; j++) {
        rowValues.push(sheetRow.getCell(left + j).value);
      }
      rows.push(rowValues);
    }
    return rows;
  ...
  set model(value) {
    ...
    this.tables = value.tables.reduce((tables, table) => {
      ...
      if (!table.rows) {
        table.rows = this._deriveTableRows(table);
      }
      // Enable filter buttons on columns only when the source `<autoFilter>` XML
      // actually had `<filterColumn>` children - never fabricate filter state
      // that wasn't present in the original file.
      if (table.columns) {
        const hasParsedFilterButtons = table.columns.some(column => column.filterButton !== undefined);
        if (hasParsedFilterButtons) {
